...
- **n8n Workflow System**: Interconnected workflows for WhatsApp message processing, including `Educare app-chat` (63 nodes) for message ingestion, user verification, intent classification, and response routing; `Lead CRM` for unregistered users; and `Inactive User Reactivation`.
    - **n8n State Machine v2.2 (Feb 2026)**: Complete 10-state conversation flow orchestrated in n8n (65 nodes): State Router v2.1 with nested API response access (`{success, state: {user_phone, state, ...}}`), 5-rule Switch routing (normal→buffer→intent, entry, feedback, exit, button_resolve), 6-rule Intent Classifier (menu, biometrics, sleep, vaccine, question, appointment), interactive context buttons (Bebê/Mãe) via Evolution API v2 format (`title/description/footer/buttons[].displayText`), button resolution API, message buffer with TTL-based concatenation (12s, min 15 chars), short-message wait prompts, and memory/feedback/context API connections. Global Constants v2.2 passes phone as-is (no normalization — backend `findUserByPhoneRobust` handles 9th digit variants). All Evolution API send nodes (Context Buttons, Feedback Buttons, Feedback Ack) use `specifyBody: json` with full payload in `jsonBody`. All HTTP POST nodes (Add To Buffer, Consume Buffer, Resolve Button, Send Wait Prompt) use `specifyBody: json`. Workflow backup: `educare-backend/docs/n8n-educare-chat-v2-state-machine.json`.
