...
- **n8n Workflow System**: Interconnected workflows for WhatsApp message processing, including `Educare app-chat` (63 nodes) for message ingestion, user verification, intent classification, and response routing; `Lead CRM` for unregistered users; and `Inactive User Reactivation`.
    - **n8n State Machine v2.3 (Feb 2026)**: Complete 10-state conversation flow orchestrated in n8n (65 nodes): State Router v2.1 with nested API response access (`{success, state: {user_phone, state, ...}}`), 5-rule Switch routing (normal→buffer→intent, entry, feedback, exit, button_resolve), 6-rule Intent Classifier (menu, biometrics, sleep, vaccine, question, appointment), interactive context buttons (Bebê/Mãe) via Evolution API v2 **flat format** (`title/description/footer/buttons[].displayText`), button resolution API, message buffer with TTL-based concatenation (12s, min 15 chars), short-message wait prompts, and memory/feedback/context API connections. Global Constants v2.2 passes phone as-is (no normalization — backend `findUserByPhoneRobust` handles 9th digit variants). All Evolution API send nodes (Context Buttons, Feedback Buttons, Feedback Ack) use `specifyBody: json` with full payload in `jsonBody`. **Button format fix (Feb 12, 2026)**: Migrated from old nested `buttonMessage.buttons[].reply.title` format to Evolution API v2 flat `buttons[].displayText` format — old format caused 400/500 errors. Workflow backup: `educare-backend/docs/n8n-educare-chat-v2-state-machine.json`.
    - **n8n Known Issue (CRITICAL)**: Intermittent "password authentication failed for user postgres" errors affecting BOTH n8n's internal PostgreSQL AND the Educare production PostgreSQL on Contabo VPS. This is a systemic VPS infrastructure issue. Affects: workflow saves, execution recording, API calls, state machine operations. End-to-end testing of the state machine is blocked by this issue. Individual components verified working during DB-available windows: user lookup (both phone formats), state CRUD, Evolution API v2 buttons, buffer APIs. Requires Contabo VPS database investigation (possibly Docker Swarm networking, PostgreSQL connection limits, or authentication configuration).
- **API Enhancements**:
...
- **AI Content Generation Fix (Feb 12, 2026)**: Fixed critical bug where AI content/media generation modules ignored user inputs (type, audience, topic, title). Root cause: when centralized prompts (from AI Agents Control Center) were active, they replaced the fallback prompt containing user parameters, and the user message sent to OpenAI was generic. Fix: (1) user inputs now always included in the `role: user` message to OpenAI, (2) inputs passed as context variables to `getProcessedPrompt` for centralized prompt injection via `{{content_type}}`, `{{target_audience}}`, `{{topic}}`, `{{resource_title}}`, `{{resource_type}}`, `{{language}}`. Affects: `contentController.generateAIContent`, `mediaResourceController.generateAIMeta`, `promptService.replaceVariables`.
- **PostgreSQL Auth Fix (Feb 11, 2026)**: Diagnostic and fix scripts for intermittent "password authentication failed" errors on Contabo VPS. Root cause: containers (backend, n8n) using desynchronized passwords for the shared `postgres` superuser. Solution: create dedicated DB users (`educare_app`, `n8n_app`) with proper permissions. Backend improved with connection retry (5 attempts with backoff), pool monitoring (`/api/admin/pool-status`), and detailed connection logging. Scripts in `educare-backend/docs/postgres-fix/`.
